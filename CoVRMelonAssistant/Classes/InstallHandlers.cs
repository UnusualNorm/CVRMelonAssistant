...
using CoVRMelonAssistant.Pages;
...
namespace CoVRMelonAssistant
{
    public static class InstallHandlers
    {
        ...
        {
            return File.Exists(Path.Combine(App.ChilloutVRInstallDirectory, "version.dll")) && File.Exists(Path.Combine(App.ChilloutVRInstallDirectory, "MelonLoader", "Dependencies", "Bootstrap.dll"));
        }
        ...
        public static bool RemoveMelonLoader()
        {
            ...
            try
            {
                var versionDllPath = Path.Combine(App.ChilloutVRInstallDirectory, "version.dll");
                var melonLoaderDirPath = Path.Combine(App.ChilloutVRInstallDirectory, "MelonLoader");
        ...
        public static async Task InstallMelonLoader()
        {
            ...
            try
            {
                ...
                {
                    var targetFileName = Path.Combine(App.ChilloutVRInstallDirectory, zipArchiveEntry.FullName);
                ...
                Directory.CreateDirectory(Path.Combine(App.ChilloutVRInstallDirectory, "Mods"));
                Directory.CreateDirectory(Path.Combine(App.ChilloutVRInstallDirectory, "Plugins"));
            }
            catch (Exception ex)
            {
