...
namespace CoVRMelonAssistant
{
    class Updater
    {
        private static readonly string APILatestURL = "https://api.github.com/repos/UnusualNorm/CoVRMelonAssistant/releases/latest";

        private static Update LatestUpdate;
        private static Version CurrentVersion;
        private static Version LatestVersion;
        private static bool NeedsUpdate = false;
        private static readonly string NewExe = Path.Combine(Path.GetDirectoryName(Utils.ExePath), "CoVRMelonAssistant.exe");
...
#pragma warning restore CS0162 // Unreachable code detected
        ...
        {
            if (Path.GetFileName(Utils.ExePath).Equals("CoVRMelonAssistant.old.exe")) RunNew();
            try
            {
        ...
        {
            string OldExe = Path.Combine(Path.GetDirectoryName(Utils.ExePath), "CoVRMelonAssistant.old.exe");
            string DownloadLink = null;

            foreach (Update.Asset asset in LatestUpdate.assets)
            {
                if (asset.name == "CoVRMelonAssistant.exe")
                {
