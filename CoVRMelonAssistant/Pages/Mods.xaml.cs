...
using Mono.Cecil;
using CoVRMelonAssistant.Libs;
using static CoVRMelonAssistant.Http;
using MessageBox = System.Windows.MessageBox;
using TextBox = System.Windows.Controls.TextBox;

namespace CoVRMelonAssistant.Pages
{
    /// <summary>
    ...
    public sealed partial class Mods : Page
    {
        ...
        private static readonly ModListItem.CategoryInfo UnknownCategory = new("Unknown/Unverified", "Mods not coming from CoVRMG. Potentially dangerous.");
        ...
        public async Task GetAllMods()
        {
            try
            {
                var resp = await HttpClient.GetAsync(Utils.Constants.CoVRMGModsJson);
        ...
        {
            if (!Directory.Exists(Path.Combine(App.ChilloutVRInstallDirectory, directory)))
            {
                return;
            }

            foreach (string file in Directory.GetFileSystemEntries(Path.Combine(App.ChilloutVRInstallDirectory, directory), "*.dll", SearchOption.TopDirectoryOnly))
            {
